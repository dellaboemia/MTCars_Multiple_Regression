...
* *Normality:* The Normal Q-Q plot in `r figr("plotlm72", TRUE, type="Figure")` indicates that distribution of standardized residuals approximates that of a sample of independent normals.  Therefore, the assumption of normality is valid for this model.
* *Constant Variance:*  The Scale-Location plot in `r figr("plotlm72", TRUE, type="Figure")` shows a horizontal line with the square root of standardized residuals dispersed evenly about the line.  The assumption of constant variance over the given values of the explanatory variables, is supported.
...
* *Bootstrap* To test the validity of the model, 1000 bootstrap samples were simulated and the sampling distribution of the model coefficients (statistic) under the null hypothesis $H_0$, was ascertained. Under $H_0$, the hypothesized model statistics were the same as the sampling distribution. As outlined in `r figr("bsum", TRUE, type="Table")` all hypothesized coefficients were within the 95% confidence interval for the statistic, as such, the null hypothesis is not rejected.     
