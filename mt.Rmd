...
This analysis posits that transmission type has no statistically significant effect on fuel efficiency when other factors such as weight and horsepower were taken into consideration.
...
* *Outliers:* The `r cd[1,2]`, `r cd[2,2]`, `r cd[3,2]`, and `r cd[4,2]` had the highest influence on the regression model with cook's distances of `r round(cd[1,1],3)`, `r round(cd[2,1],3)`, `r round(cd[3,1],3)`, and `r round(cd[4,1],3)`, respectively.  All points were lower than the cut-off value of $D_i > 1$.

Lotus Europe observation was the highest **leverage** point at roughly `r max(lev)`.
The Chrysler Imperial, Fiat 128, and Toyota Corolla wereand Lotus Europa were four potential outliers observed. However, the Residuals vs. Leverage in `r figr("plotlm72", TRUE, type="Figure")` reveals that these points are well within the Cook's distance and; therefore, exert minimal influence. 
